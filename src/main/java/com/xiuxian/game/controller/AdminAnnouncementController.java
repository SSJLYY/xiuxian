...
import com.xiuxian.game.service.AnnouncementService;
...
public class AdminAnnouncementController {
    ...
    public ResponseEntity<ApiResponse<Announcement>> createAnnouncement(
        ...
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_CREATE_ANNOUNCEMENT", 
                    "管理员创建公告: title=" + request.getTitle());
            LogUtils.logBusiness("ADMIN_ANNOUNCEMENT", "管理员创建公告", 
                    "adminId", adminId, "announcementId", announcement.getId(), 
                    "type", request.getAnnouncementType());
            
            log.info("管理员创建公告成功: adminId={}, announcementId={}", adminId, announcement.getId());
            
            return ResponseEntity.ok(ApiResponse.success("创建成功", announcement));
            
        } catch (Exception e) {
            log.error("管理员创建公告失败: {}", e.getMessage(), e);
    ...
    public ResponseEntity<ApiResponse<Announcement>> updateAnnouncement(
        ...
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_UPDATE_ANNOUNCEMENT", 
                    "管理员更新公告: announcementId=" + id);
            LogUtils.logBusiness("ADMIN_ANNOUNCEMENT", "管理员更新公告", 
                    "adminId", adminId, "announcementId", id);
            
            log.info("管理员更新公告成功: adminId={}, announcementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("更新成功", announcement));
            
        } catch (Exception e) {
            log.error("管理员更新公告失败: announcementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteAnnouncement(@PathVariable Long id) {
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_DELETE_ANNOUNCEMENT", 
                    "管理员删除公告: announcementId=" + id);
            LogUtils.logBusiness("ADMIN_ANNOUNCEMENT", "管理员删除公告", 
                    "adminId", adminId, "announcementId", id);
            
            log.info("管理员删除公告成功: adminId={}, announcementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("删除成功", null));
            
        } catch (Exception e) {
            log.error("管理员删除公告失败: announcementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> publishAnnouncement(@PathVariable Integer id) {
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_PUBLISH_ANNOUNCEMENT", 
                    "管理员发布公告: announcementId=" + id);
            LogUtils.logBusiness("ADMIN_ANNOUNCEMENT", "管理员发布公告", 
                    "adminId", adminId, "announcementId", id);
            
            log.info("管理员发布公告成功: adminId={}, announcementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("发布成功", null));
            
        } catch (Exception e) {
            log.error("管理员发布公告失败: announcementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> revokeAnnouncement(@PathVariable Integer id) {
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_REVOKE_ANNOUNCEMENT", 
                    "管理员撤回公告: announcementId=" + id);
            LogUtils.logBusiness("ADMIN_ANNOUNCEMENT", "管理员撤回公告", 
                    "adminId", adminId, "announcementId", id);
            
            log.info("管理员撤回公告成功: adminId={}, announcementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("撤回成功", null));
            
        } catch (Exception e) {
            log.error("管理员撤回公告失败: announcementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<IPage<Announcement>>> getAllAnnouncements(
        ...
        try {
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_GET_ANNOUNCEMENT_LIST", 
                    "管理员获取公告列表: page=" + page + ", size=" + size);
            
            log.debug("管理员获取公告列表成功: adminId={}, total={}", adminId, announcements.getTotal());
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", announcements));
            
        } catch (Exception e) {
            log.error("管理员获取公告列表失败: {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Announcement>> getAnnouncementDetail(@PathVariable Long id) {
        try {
            ...
            log.debug("管理员获取公告详情成功: adminId={}, announcementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", announcement));
            
        } catch (Exception e) {
            log.error("管理员获取公告详情失败: announcementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Long>>> getAnnouncementStats() {
        try {
            ...
            log.debug("管理员获取公告统计成功: adminId={}, totalCount={}", 
                    adminId, stats.getTotalCount());
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", result));
            
        } catch (Exception e) {
            log.error("管理员获取公告统计失败: {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
