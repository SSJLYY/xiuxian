...
public class GameCalculator {
    ...
    private static final Map<String, BigDecimal> REALM_BONUS = Java8Compatibility.mapOf(
        "练气境界", BigDecimal.ZERO,
        "筑基境界", BigDecimal.valueOf(0.5),
        "金丹境界", BigDecimal.ONE,
        "元婴境界", BigDecimal.valueOf(2),
        "化神境界", BigDecimal.valueOf(4),
        "合体境界", BigDecimal.valueOf(8),
        "大乘境界", BigDecimal.valueOf(16),
        "渡劫境界", BigDecimal.valueOf(32)
    );
    ...
    public long calculateExpPerSecond(PlayerProfile player) {
        ...
        // 确保cultivationSpeed不为null，否则使用默认值1.0
    ...
    private void updateRealm(PlayerProfile player) {
        ...
        else if (level >= 701) player.setRealm("化神境界");
        else if (level >= 401) player.setRealm("元婴境界");
        else if (level >= 201) player.setRealm("金丹境界");
        else if (level >= 101) player.setRealm("筑基境界");
        else player.setRealm("练气境界");
    }
    ...
    public long calculateOfflineRewards(PlayerProfile player, long offlineSeconds) {
        ...
        if (effectiveTime < 60) return 0; // 小于1分钟无收益
    ...
    public long calculateSkillExpGain(int skillLevel, String skillType) {
        ...
        switch (skillType) {
            case "攻击":
                return baseExp * skillLevel; // 攻击技能经验较大
            case "防御":
                return Math.round(baseExp * skillLevel * 0.8); // 防御技能经验稍小
            case "辅助":
                return Math.round(baseExp * skillLevel * 0.6); // 辅助技能经验最小
            default:
    ...
    public double calculateLevelEffectBonus(int playerLevel, int skillLevel) {
        // 修士等级越高，技能效果越好
        double levelBonus = Math.min(playerLevel * 0.01, 2.0); // 最大100%加成
