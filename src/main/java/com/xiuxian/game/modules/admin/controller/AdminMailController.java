...
public class AdminMailController {
    ...
    public ResponseEntity<ApiResponse<Void>> sendMail(@Valid @RequestBody SendMailRequest request) {
        try {
            ...
            log.info("管理员发送邮�? adminId={}, playerId={}, title={}", 
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_SEND_MAIL", 
                    "管理员发送邮�? playerId=" + request.getPlayerId() + ", title=" + request.getTitle());
            LogUtils.logBusiness("ADMIN_MAIL", "管理员发送邮�?, 
                    "adminId", adminId, "playerId", request.getPlayerId(), "mailType", request.getMailType());
            
            log.info("管理员发送邮件成�? adminId={}, playerId={}", adminId, request.getPlayerId());
            
            return ResponseEntity.ok(ApiResponse.success("发送成�?, null));
            
        } catch (Exception e) {
            log.error("管理员发送邮件失�? {}", e.getMessage(), e);
    ...
    public ResponseEntity<ApiResponse<Void>> sendBatchMail(@Valid @RequestBody SendBatchMailRequest request) {
        try {
            ...
            log.info("管理员批量发送邮�? adminId={}, playerCount={}, title={}", 
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_SEND_BATCH_MAIL", 
                    "管理员批量发送邮�? playerCount=" + request.getPlayerIds().size() + ", title=" + request.getTitle());
            LogUtils.logBusiness("ADMIN_MAIL", "管理员批量发送邮�?, 
                    "adminId", adminId, "playerCount", request.getPlayerIds().size(), "mailType", request.getMailType());
            
            log.info("管理员批量发送邮件成�? adminId={}, playerCount={}", 
                    adminId, request.getPlayerIds().size());
            
            return ResponseEntity.ok(ApiResponse.success("批量发送成�?, null));
            
        } catch (Exception e) {
            log.error("管理员批量发送邮件失�? {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public static class SendMailRequest {
        ...
        private Integer expireDays; // 过期天数，默�?�?
    }
