...
public class AdminAchievementController {
    ...
    public ResponseEntity<ApiResponse<Achievement>> createAchievement(
        ...
        try {
            ...
            log.info("管理员创建成�? adminId={}, name={}", adminId, request.getName());
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_CREATE_ACHIEVEMENT", 
                    "管理员创建成�? name=" + request.getName());
            LogUtils.logBusiness("ADMIN_ACHIEVEMENT", "管理员创建成�?, 
                    "adminId", adminId, "achievementId", achievement.getId(), 
                    "type", request.getAchievementType());
            
            log.info("管理员创建成就成�? adminId={}, achievementId={}", adminId, achievement.getId());
            
            return ResponseEntity.ok(ApiResponse.success("创建成功", achievement));
            
        } catch (Exception e) {
            log.error("管理员创建成就失�? {}", e.getMessage(), e);
    ...
    public ResponseEntity<ApiResponse<Achievement>> updateAchievement(
        ...
        try {
            ...
            log.info("管理员更新成�? adminId={}, achievementId={}", adminId, id);
            
            Achievement achievement = achievementMapper.selectById(id);
            if (achievement == null) {
                return ResponseEntity.badRequest().body(ApiResponse.error("成就不存�?));
            }
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_UPDATE_ACHIEVEMENT", 
                    "管理员更新成�? achievementId=" + id);
            LogUtils.logBusiness("ADMIN_ACHIEVEMENT", "管理员更新成�?, 
                    "adminId", adminId, "achievementId", id);
            
            log.info("管理员更新成就成�? adminId={}, achievementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("更新成功", achievement));
            
        } catch (Exception e) {
            log.error("管理员更新成就失�? achievementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
     ...
     * <ul>
     *   <li>删除后无法恢�?/li>
     *   <li>会同时删除所有玩家的该成就进度记�?/li>
     *   <li>建议只删除测试或错误创建的成�?/li>
    ...
    public ResponseEntity<ApiResponse<Void>> deleteAchievement(@PathVariable Integer id) {
        try {
            ...
            log.info("管理员删除成�? adminId={}, achievementId={}", adminId, id);
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_DELETE_ACHIEVEMENT", 
                    "管理员删除成�? achievementId=" + id);
            LogUtils.logBusiness("ADMIN_ACHIEVEMENT", "管理员删除成�?, 
                    "adminId", adminId, "achievementId", id);
            
            log.info("管理员删除成就成�? adminId={}, achievementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("删除成功", null));
            
        } catch (Exception e) {
            log.error("管理员删除成就失�? achievementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<IPage<Achievement>>> getAllAchievements(
        ...
        try {
            ...
            log.debug("管理员获取成就列�? adminId={}, page={}, size={}, type={}", 
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_GET_ACHIEVEMENT_LIST", 
                    "管理员获取成就列�? page=" + page + ", size=" + size);
            
            log.debug("管理员获取成就列表成�? adminId={}, total={}", adminId, achievements.getTotal());
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", achievements));
            
        } catch (Exception e) {
            log.error("管理员获取成就列表失�? {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Achievement>> getAchievementDetail(@PathVariable Integer id) {
        try {
            ...
            log.debug("管理员获取成就详�? adminId={}, achievementId={}", adminId, id);
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_GET_ACHIEVEMENT_DETAIL", 
                    "管理员查看成就详�? achievementId=" + id);
            
            log.debug("管理员获取成就详情成�? adminId={}, achievementId={}", adminId, id);
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", achievement));
            
        } catch (Exception e) {
            log.error("管理员获取成就详情失�? achievementId={}, error={}", id, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getAchievementStats() {
        try {
            ...
            log.debug("管理员获取成就统�? adminId={}", adminId);
            ...
            log.debug("管理员获取成就统计成�? adminId={}, totalCount={}", adminId, totalCount);
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", result));
            
        } catch (Exception e) {
            log.error("管理员获取成就统计失�? {}", e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<PlayerAchievement>>> getPlayerAchievements(
        ...
        try {
            ...
            log.debug("管理员查看玩家成�? adminId={}, playerId={}", adminId, playerId);
            ...
            LogUtils.logUserAction(null, adminId, "ADMIN_GET_PLAYER_ACHIEVEMENTS", 
                    "管理员查看玩家成�? playerId=" + playerId);
            
            log.debug("管理员查看玩家成就成�? adminId={}, playerId={}, count={}", 
                    adminId, playerId, playerAchievements.size());
            
            return ResponseEntity.ok(ApiResponse.success("获取成功", playerAchievements));
            
        } catch (Exception e) {
            log.error("管理员查看玩家成就失�? playerId={}, error={}", playerId, e.getMessage(), e);
            return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
        }
    }
    ...
    public static class CreateAchievementRequest {
        ...
        @NotNull(message = "条件数值不能为�?)
        @Min(value = 1, message = "条件数值必须大�?")
