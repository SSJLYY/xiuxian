...
public class AsyncMailService {
    ...
    public CompletableFuture<Void> sendMailBatch(List<Integer> playerIds, String title, 
        ...
        try {
            log.info("开始批量发送邮�? 目标玩家�?{}, 标题={}", playerIds.size(), title);
            ...
            for (Integer playerId : playerIds) {
                try {
                    ...
                    // 每发�?00封邮件休息一下，避免过度占用资源
                    if (successCount % 100 == 0) {
                        Thread.sleep(100);
                        log.debug("已发�?{} 封邮�?, successCount);
                    }
                    
                } catch (Exception e) {
                    failCount++;
                    log.warn("发送邮件失�? playerId={}, error={}", playerId, e.getMessage());
                }
            }
            
            log.info("批量邮件发送完�? 成功={}, 失败={}", successCount, failCount);
            
        } catch (Exception e) {
            log.error("批量发送邮件异�?, e);
            // 异步方法中通过 CompletableFuture 传递异常；此处 re-wrap 确保调用方能感知到失�?
    ...
    public CompletableFuture<Void> sendMailAsync(Integer playerId, String title, String content, 
        ...
        try {
            mailService.sendMail(playerId, title, content, mailType, attachments, expireAt);
            log.debug("异步邮件发送成�? playerId={}, title={}", playerId, title);
        } catch (Exception e) {
            log.error("异步邮件发送失�? playerId={}, title={}", playerId, title, e);
    ...
    public CompletableFuture<Void> sendSystemNotificationAsync(List<Integer> playerIds, 
        ...
        try {
            log.info("开始发送系统通知邮件: 目标玩家�?{}, 标题={}", playerIds.size(), title);
    ...
    public CompletableFuture<Void> sendActivityRewardAsync(List<Integer> playerIds, String title, 
        ...
        try {
            log.info("开始发送活动奖励邮�? 目标玩家�?{}, 标题={}", playerIds.size(), title);
            
            LocalDateTime expireAt = LocalDateTime.now().plusDays(30); // 30天后过期
            
            int batchSize = 50; // 每批处理50个玩�?
            for (int i = 0; i < playerIds.size(); i += batchSize) {
                ...
                for (Integer playerId : batchPlayerIds) {
                    try {
                        ...
                        log.warn("发送活动奖励失�? playerId={}, error={}", playerId, e.getMessage());
                    }
                }
