...
class App {
    ...
    onPageUnload() {
        // 清理定时器
        if (gameManager && gameManager.cultivationTimer) {
    ...
    saveGameState() {
        try {
            const gameState = {
                timestamp: Date.now(),
                player: authManager ? authManager.player : null,
                cultivationTime: gameManager ? gameManager.cultivationTime : 0,
                isCultivating: gameManager ? gameManager.isCultivating : false
            };
    ...
    loadGameState() {
        try {
            ...
            if (savedState) {
                ...
                // 恢复修炼状态
                if (gameState.isCultivating && gameManager) {
    ...
    // 更新UI
    updateUI() {
        if (authManager && authManager.isAuthenticated && authManager.player) {
            // 更新修炼时间显示
            if (gameManager && gameManager.isCultivating) {
